...
export class MilestoneDataService {
	...
	async fetchData(
		MSRMngCode: string,
		skipNum: number,
	): Promise<MSRAIPDataType[]> {
		const APIUrl = MSR_API_URL.MSRGetAIPData.replace('%1', MSRMngCode).replace(
			'%2',
			skipNum.toString(),
		);
	...
	async refreshPIPGroup(
		MSRMngCode: string,
		PIPCode: string,
		currentData: MSRAIPDataType[],
	): Promise<MSRAIPDataType[]> {
...
export function useMilestoneDataService({
	MSRMngCode,
	pageSize = 50,
}: MilestoneDataServiceOptions) {
	...
	const loadMoreData = useCallback(async () => {
		...
		try {
			const newData = await service.fetchData(
				MSRMngCode,
				state.skipNum + pageSize,
			);
	...
	// Refresh specific PIP group
	const refreshPIPGroup = useCallback(
		async (PIPCode: string) => {
			if (!MSRMngCode) return;

			try {
				const updatedData = await service.refreshPIPGroup(
					MSRMngCode,
					PIPCode,
					state.data,
				);
